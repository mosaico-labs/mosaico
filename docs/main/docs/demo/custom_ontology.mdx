...
sidebar_position: 3
description: "Runnable demo showing how to extend Mosaico with custom data models for hardware not covered by the built-in ontology. Covers Serializable subclassing, MosaicoField and MosaicoType declarations, automatic schema registration via module import, and verifying registration status."
---

Demonstrates how to define and register a custom data model for the NVIDIA Isaac Nova wheel encoder, a hardware type not present in the built-in Mosaico ontology. This is the ontology code that underpins the [ROS Ingestion](./ros_ingestion) demo.
...
The custom ontology is loaded as part of the ROS Ingestion demo. The full source is on [GitHub](https://github.com/mosaico-labs/mosaico/blob/main/mosaico-sdk-py/src/mosaicolabs/examples/ros_injection/custom_ontology).
