...
sidebar_position: 2
description: "Runnable demo showing how to browse a Mosaico catalog without downloading bulk data. Covers listing sequences, reading session metadata (size, timestamps) via SequenceHandler, and inspecting individual Topics via TopicHandler using the lazy handler pattern."
---
...
The daemon must be running and contain at least one ingested sequence. Run the [ROS Ingestion](./ros_ingestion) demo first to populate it. The full source is on [GitHub](https://github.com/mosaico-labs/mosaico/blob/main/mosaico-sdk-py/src/mosaicolabs/examples/data_inspection.py).
