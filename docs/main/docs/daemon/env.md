...
- `MOSAICOD_CLEANUP_TIME_INTERVAL`: Specifies (in seconds) how often the routine checks whether it should run or not. 
Set it to 0 if you want to disable the cleanup routine. Default is once a day.
- `MOSAICOD_CLEANUP_RETENTION_DURATION`: How long (seconds) a folder is retained before being permanently deleted.
Set it to 0 if you want to delete it right away. Default is 1 day.