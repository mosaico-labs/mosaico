...
- <a id="mosaicod-max-grpc-decode-message-size"></a>`MOSAICOD_MAX_GRPC_DECODE_MESSAGE_SIZE`: The maximum allowed size, in bytes, of an incoming [gRPC](https://grpc.io/) message (the server's decode limit). If a message exceeds this size, a protocol error will be returned. Default is `50 MiB`. Must be between `4 MiB` and `512 MiB`. If you need to update this value be aware that it is tipically smaller than `MOSAICOD_PARQUET_IN_MEMORY_ENCODING_BUFFER_SIZE`. The server's outgoing message size is fixed independently of this value, so lowering it later never affects the retrievability of previously-accepted data.

- <a id="mosaicod-target-grpc-encode-message-size"></a>`MOSAICOD_TARGET_GRPC_ENCODE_MESSAGE_SIZE`: Target message size in bytes used during data streaming. The daemon will try to aggregate a number of [`RecordBatches`](https://arrow.apache.org/docs/python/generated/pyarrow.RecordBatch.html) to create a sufficiently large message. If the resulting batch size exceeds the limit, it will be capped by `MOSAICOD_MAX_BATCH_SIZE`. Defaults to `25 MiB`.
