...
This guide demonstrates how to ingest data into Mosaico. The example uses a CSV file as the data source, chosen purely for its simplicity: no dependencies, no binary tooling, just text you can open in any editor. The CSV is a teaching device, not a recommendation.

:::info
In production, your source data will almost certainly come from a binary container rather than a CSV. 
Mosaico's SDK is format-agnostic: it does not care where your `Message` objects come from. 
...
With the `TopicWriter` in hand, you iterate over the **generator** and push each message. The SDK serializes each `Message` using Apache Arrow, batches it with preceding messages, and transmits batches to the daemon over gRPC. From your perspective, the call is a simple method invocation; the network and serialization complexity is hidden.
...
**Topic-Level Error Management**: The **`try-except`** around each `push()` call is important. If an error occurs at the push level, for example a network hiccup or a type validation failure for a particular message, catching it here prevents it from propagating up to the `SequenceWriter`'s **context manager**. If an unhandled exception reaches the sequence level, the `on_error` policy fires and potentially deletes everything you have already uploaded. By catching errors at the topic level, you can log the problematic timestamp and continue ingesting the rest of the data, letting the sequence complete successfully.
