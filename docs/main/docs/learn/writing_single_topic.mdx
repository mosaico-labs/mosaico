...
A **Message** is the in-memory wrapper you hand to the SDK. It couples a typed data payload (your `IMU`, `GPS`, or `Pressure` object) with a monotonic timestamp. The SDK serializes the Message and transmits it to the daemon over gRPC. You never deal with bytes directly; you construct typed objects, wrap them in `Message`, and let the SDK handle the rest.

:::info
The timestamp is what Mosaico uses to order and index records in time and it is therefore fundamental that all timestamps' measurements of the same **Sequence** have a common origin. 
Therefore, this does not indicate the sensor measurement timestamp but rather the time the data distribution system (like the ROS DDS) makes the data available to everyone.
The measurement timestamp is stored within the `timestamp` contained in the `Header` object that most of the Mosaico Ontologies contain. Notice that different sensors may have different origin time reference 
(like time since Epoch or sensor startup), meaning that such timestamp cannot be exploited to index records within Mosaico.
In short, hereafter the main differences between the `Header` timestamp within an `Ontology` and `Message` timestamp:
...
| Common start within **Sequence**   | Yes               | No                                             |
