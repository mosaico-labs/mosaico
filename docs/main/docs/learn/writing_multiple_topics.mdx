...
For this alignment to hold, every `Message.timestamp_ns` across all topics in a Sequence must share a common origin. It represents the time the data distribution system (e.g. ROS DDS) made the message available, not the sensor measurement time. For the full explanation and a comparison table, see the [Writing a Single Topic](./writing_single_topic.mdx#understanding-the-mosaico-data-model) guide.
:::
