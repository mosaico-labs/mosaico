...
With all three writers registered, we iterate over each generator and push its messages. The three `for` loops run one after the other: first all IMU messages, then all GPS messages, then all Pressure messages. Each loop is independent and isolated from the others by its own `try-except` around each push call.

This per-topic error isolation is the critical design choice in multi-topic ingestion. If an exception escapes a push call and is not caught at the topic level, it will propagate up through the generator loop, exit the `with swriter:` block abnormally, and trigger the sequence-level `on_error` policy, potentially deleting everything. By wrapping each `push()` call in a `try-except`, you contain errors to the individual message that caused them. The affected timestamp is logged, and ingestion continues with the next message. The GPS and Pressure loops are completely unaffected by any errors in the IMU loop. Each sensor's data reaches the daemon independently, and the sequence is finalized successfully at the end.
...
for msg in stream_imu_from_csv("imu.csv"):
    ...
    try:
        ...
        print(f"IMU error at {msg.timestamp_ns}: {e}")
...
for msg in stream_gps_from_csv("gps.csv"):
    ...
    try:
        ...
        print(f"GPS error at {msg.timestamp_ns}: {e}")
...
for msg in stream_pressure_from_csv("pressure.csv"):
    ...
    try:
        ...
        print(f"Pressure error at {msg.timestamp_ns}: {e}")
```

**Topic-Level Error Management**: The `try-except` around each `push()` call catches exceptions at the message level and logs the nanosecond timestamp, giving you enough context to trace the failure back to a row in the source CSV. Individual bad records do not abort the session. When you later query the platform, you can compare the logged error timestamps against the ingested data to understand exactly which records are missing. This is far more actionable than a session-level abort that leaves you with nothing.
