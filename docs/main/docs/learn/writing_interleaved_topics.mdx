...
:::info[ROS `header.stamp` and the Message timestamp]
In ROS, `header.stamp` records **when the sensor took the measurement**. It is the sensor measurement timestamp, equivalent to the `Header` timestamp inside a Mosaico Ontology. It is **not** appropriate for `Message.timestamp_ns`, which must be a monotonic timestamp with a common origin across the whole Sequence (i.e. the DDS publish time). In MCAP, the correct value for `Message.timestamp_ns` is `message.publish_time`, available from the MCAP message object in the ingestion loop. The translator below therefore accepts `publish_time_ns` as an explicit parameter instead of extracting a timestamp from the payload. For a full explanation of this distinction, see the [Writing a Single Topic](./writing_single_topic.mdx#understanding-the-mosaico-data-model) guide.
:::
...
from mosaicolabs import Message, IMU, GPS, Pressure, Vector3d, GPSStatus, Point3d

def custom_translator(schema_name: str, payload: dict, publish_time_ns: int):
    if schema_name == "sensor_msgs/msg/Imu":
        return Message(timestamp_ns=publish_time_ns, data=IMU(
    ...
    if schema_name == "sensor_msgs/msg/NavSatFix":
        return Message(timestamp_ns=publish_time_ns, data=GPS(
    ...
    if schema_name == "sensor_msgs/msg/FluidPressure":
        return Message(timestamp_ns=publish_time_ns, data=Pressure(value=payload['fluid_pressure']))
    return None
                ...
                for schema, channel, message in reader.iter_messages():
                    ...
                    if twriter.is_active:
                        with twriter:
                            ...
                            msg = custom_translator(schema.name, raw, message.publish_time)
                            if msg is None:
                                continue
...
from mcap.reader import make_reader
from mosaicolabs import MosaicoClient, setup_sdk_logging, SessionLevelErrorPolicy, TopicLevelErrorPolicy
from mosaicolabs import IMU, GPS, Pressure, Vector3d, GPSStatus, Point3d
