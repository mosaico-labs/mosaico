...
`TopicLevelErrorPolicy.Finalize` closes the affected topic writer on error, allowing the remaining topics in the sequence to continue ingesting. Without this policy, a single bad message could trigger the sequence-level error policy and abort the entire upload. For interleaved ingestion where multiple sensors share a single chronological stream, this policy is strongly recommended.
:::
