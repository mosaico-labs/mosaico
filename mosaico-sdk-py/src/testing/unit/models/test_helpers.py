...
def test_resolve_same_tag_same_schema_is_idempotent():
    ...
    # (which would have raised "Duplicate ontology tag" during registration anyway).
...
def test_resolve_schema_variant_creates_distinct_class():
    ...
    assert variant.__ontology_tag__ == f"{tag}__{variant.__schema_fingerprint__}"
...
def test_resolve_wrong_precomputed_fingerprint_is_honored_over_recomputation():
    ...
    assert resolved.__ontology_tag__ == f"{tag}__{bogus_fingerprint}"
...
def test_resolve_ontology_class_is_thread_safe():
    ...
    # with no "Duplicate ontology tag" exception from a lost race.
