...
def test_loader_filter(mcap_file):
    ...
    assert len(mcap_loader_base.topics) == 4
    assert len(mcap_loader_base.resolved_topics) == 4
    ...
    assert len(mcap_loader_filtered.filtered_topics) == 3
    ...
    mcap_loader_filtered = MCAPLoader(mcap_file, ["*", "!*Imu*"])
    assert len(mcap_loader_filtered.topics) == 3
