                ...
                search_jobs.push(async move {
                    ...
                    for chunk in chunks {
                        ...
                        if topic.is_none() {
                            ...
                            return Ok::<_, Error>(types::SequenceTopicGroupSet::empty());
                        }
...
type TopicMap = HashMap<i32, db::TopicRecord>;
...
async fn pre_fetch_topics(
    ...
    let topic_map = if let Some(topics) = on_topics {
        ...
        let fetched_topics = db::topic_find_by_ids(cx, &chunk_topic_ids).await?;
        fetched_topics
            .into_iter()
            .map(|t| (t.topic_id, t))
            .collect()
