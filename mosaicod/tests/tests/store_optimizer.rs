...
/// Tests the optimization in a scenario with 1 sequence, 2 topics and many record batches.
...
async fn test_store_optimization_3(pool: sqlx::Pool<db::DatabaseType>) {
    ...
    let max_file_size = 50000000; // 50 MB
    ...
    // Wait for the optimizer to run the first time.
