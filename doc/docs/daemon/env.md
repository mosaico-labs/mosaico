...
- `MOSAICOD_MAX_MESSAGE_SIZE_IN_BYTES`: The maximum allowed [gRPC](https://grpc.io/) message size in bytes. If a message exceeds this size, a protocol error will be returned. Default is `50 MiB`.

- `MOSAICOD_TARGET_MESSAGE_SIZE_IN_BYTES`: Target message size used during data streaming. The daemon will try to aggregate a number of [`RecordBatches`](https://arrow.apache.org/docs/python/generated/pyarrow.RecordBatch.html) to create a sufficiently large message. If the resulting batch size exceeds the limit, it will be capped by `MOSAICOD_MAX_BATCH_SIZE`. Defaults to `25MiB`.
...
- `MOSAICOD_MAX_CHUNK_SIZE_IN_BYTES`: Maximum chunk size in bytes before automatic splitting during upload. When a chunk exceeds this size, it is serialized on the store and a new chunk is started. A value of `0` means unlimited chunk size and no automatic splitting. Defaults to `250 MiB`. 
